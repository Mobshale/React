...
class Chat extends React.Component {
  constructor(props) {
    ...
    const postListRef = ref(db, time + "/members/" + roomName);
  ...
  componentDidMount() {
    const db = getDatabase();
    const chatRef = ref(db, time + "/chats/" + roomName);
    ...
    const memRef = ref(db, time + "/members/" + roomName);
  ...
  send = () => {
    ...
    if (value != "") {
      ...
      const db = getDatabase();
      const postListRef = ref(db, time + "/chats/" + roomName);
      const newPostRef = push(postListRef);
      set(newPostRef, {
        text: value,
        uid: "2",
