...
class Pollprogress extends React.Component{
    ...
    componentDidMount(){
      const Ref  = ref(FirebaseDatabase, "poll/"+roomN+"/hits");
      onValue(Ref,(snap) => {
        const Data = snap.val();
        if(Data!=null){
          if(sec==null){
          ...
          <div class="progress" style={this.state.showMyComponent ? {} : { display: 'none' }}>
             <Card title="Poll progress"  bordered={false} extra={<div class="secc">
                 {this.state.x}
                  ...
                  style={{ width: 300 }}>
             <div class="sec"><p>A</p> <Progress percent={this.state.aPercent} status="active" /></div>
             <div class="sec"><p>B</p> <Progress percent={this.state.bPercent} status="active" /></div>
             <div class="sec"><p>C</p> <Progress percent={this.state.cPercent} status="active" /></div>
             <div class="sec"><p>D</p> <Progress percent={this.state.dPercent} status="active" /></div>
             <Button type="primary" disabled="true" onClick={() => this.setState({setopen: true})} block>Leaderboard</Button>
             </Card>
             <Modal
             title="Leaderboard"
             centered
